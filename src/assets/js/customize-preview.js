...
wp.customize( 'jjlazo_display_author_info', (value) => {
    value.bind( (to) => {
        if(to) {
            $('.c-post-author').show();
        } else {
...
wp.customize( 'jjlazo_accent_colour', (value) => {
    value.bind( (to) => {
        let inline_css = ``;
        let inline_css_obj = jjlazo['inline-css'];
        ...
        $('#jjlazo-stylesheet-inline-css').html(inline_css);
    } )
})

wp.customize( 'jjlazo_site_info', (value) => {
    value.bind( (to) => {
