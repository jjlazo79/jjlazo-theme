...
export const replace_filenames = () => {
  return gulp
    ...
    .pipe(
      rename(path => {
        path.basename = path.basename.replace("jjlazo", info.name);
      })
    )
    .pipe(gulp.dest("./"));
};
...
export const delete_replaced_filenames = () => {
  return del(
    paths.rename.src.map(filename => filename.replace("jjlazo", info.name))
  );
};

export const serve = done => {
  server.init({
    proxy: "http://joselazo.test/"
  });
  done();
};
