...
function jjlazo_content_width()
{
	global $content_width;
	global $post;

	if (is_single() && $post->post_type === 'post') {
		$layout = jjlazo_meta($post->ID, '_jjlazo_post_layout', 'full');
...
add_action('template_redirect', 'jjlazo_content_width');

function jjlazo_image_sizes($sizes, $size, $image_src, $image_meta, $attachment_id)
{
	$width = $size[0];
	global $content_width;
	global $post;
...
add_filter('wp_calculate_image_sizes', 'jjlazo_image_sizes', 10, 5);
